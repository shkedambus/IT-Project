...
def jira_2(ack, body, view, logger):
    ...
    email = myquery["email"]
    db["temp"].drop()
    my_db.update_db("jira", {"domain": domain, "api_key": api_key, "email": email, "project": selected_project}, False)
...
def jira_3(ack, body, view, logger):
    ...
    return my_db.update_db("users", newvalues, True) #обновление базы данных
...
def user_emoji(ack, body, view, logger):
    ...
    my_db.update_db("reactions", data, True)
